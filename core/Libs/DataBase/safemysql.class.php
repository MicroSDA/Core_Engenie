...
class SafeMySQL
{
    ...
    protected function setConnection()
    {
        ...
        while ($xml_parse_doc->read()) {

            if ($xml_parse_doc->nodeType == XMLReader::ELEMENT) {

                if ($xml_parse_doc->localName == 'Host') {

                    $xml_parse_doc->read();

                    if ($xml_parse_doc->nodeType == XMLReader::TEXT) {

                        $this->defaults['host'] = $xml_parse_doc->value;

                    } else {

                        throw new ErrorException('DATA BASE HOST IS NOT FOUND');
                    }

                }

                if ($xml_parse_doc->localName == 'User') {

                    $xml_parse_doc->read();

                    if ($xml_parse_doc->nodeType == XMLReader::TEXT) {

                        $this->defaults['user'] = $xml_parse_doc->value;

                    } else {

                        throw new ErrorException('DATA BASE USER IS NOT FOUND');
                    }

                }

                if ($xml_parse_doc->localName == 'Password') {

                    $xml_parse_doc->read();

                    if ($xml_parse_doc->nodeType == XMLReader::TEXT) {

                        $this->defaults['pass'] = $xml_parse_doc->value;
                ...
                if ($xml_parse_doc->localName == 'DB-Name') {

                    $xml_parse_doc->read();

                    if ($xml_parse_doc->nodeType == XMLReader::TEXT) {

                        $this->defaults['db'] = $xml_parse_doc->value;

                    } else {

                        throw new ErrorException('DATA BASE NAME IS NOT FOUND');
                    }

                }

                if ($xml_parse_doc->localName == 'Port') {

                    $xml_parse_doc->read();

                    if ($xml_parse_doc->nodeType == XMLReader::TEXT) {

                        if($xml_parse_doc->value == 'NULL'){

                            $this->defaults['port'] = NULL;
                        }else{

                            $this->defaults['port'] = $xml_parse_doc->value;
                        }


                    } else {

                        throw new ErrorException('DATA BASE PORT IS NOT FOUND');
                    }

                }

                if ($xml_parse_doc->localName == 'Socket') {
                    ...
                    if ($xml_parse_doc->nodeType == XMLReader::TEXT) {
                        ...
                        if($xml_parse_doc->value == 'NULL'){
                            ...
                            $this->defaults['socket'] = NULL;
                        }else{

                            $this->defaults['socket'] = $xml_parse_doc->value;
                        }

                    } else {

                        throw new ErrorException('DATA BASE SOCKET IS NOT FOUND');
                    }

                }

                if ($xml_parse_doc->localName == 'Pconnect') {

                    $xml_parse_doc->read();

                    if ($xml_parse_doc->nodeType == XMLReader::TEXT) {

                        if($xml_parse_doc->value == 'FALSE'){

                            $this->defaults['pconnect'] = FALSE;
                        }else{

                            $this->defaults['pconnect']=$xml_parse_doc->value;
                        }


                    } else {

                        throw new ErrorException('DATA BASE PCONNECT PORT IS NOT FOUND');
                    }

                }

                if ($xml_parse_doc->localName == 'Charset') {

                    $xml_parse_doc->read();

                    if ($xml_parse_doc->nodeType == XMLReader::TEXT) {

                        $this->defaults['charset'] = $xml_parse_doc->value;

                    } else {

                        throw new ErrorException('DATA BASE CHARSET PORT IS NOT FOUND');
                    }

                }

                if ($xml_parse_doc->localName == 'Errmode') {

                    $xml_parse_doc->read();

                    if ($xml_parse_doc->nodeType == XMLReader::TEXT) {

                        $this->defaults['errmode'] = $xml_parse_doc->value;

                    } else {

                        throw new ErrorException('DATA BASE ERRMODE IS NOT FOUND');
                    }

                }

                if ($xml_parse_doc->localName == 'Exception') {

                    $xml_parse_doc->read();

                    if ($xml_parse_doc->nodeType == XMLReader::TEXT) {

                        $this->defaults['exception'] = $xml_parse_doc->value;

                    } else {

                        throw new ErrorException('DATA BASE EXCEPTION IS NOT FOUND');
                    }
